...
var _f = class _f {
  constructor(e7) {
    ...
    for (let a6 = 0; a6 < this.levels.length; a6++) this.levels[a6] = { resolution: n6, scale: o3, tileSize: [n6 * i4.size[0], n6 * i4.size[1]] }, n6 /= 2, o3 /= 2;
  }
  clone() {
  ...
  rootTilesInExtent(e7, t4 = null, i4 = 1 / 0) {
    ...
    let l5 = v[1], n6 = v[3], o3 = v[0], a6 = v[2];
    const h5 = a6 - o3, c6 = n6 - l5;
    if (h5 * c6 > i4) {
      const e8 = Math.floor(Math.sqrt(i4));
      c6 > e8 && (l5 = l5 + Math.floor(0.5 * c6) - Math.floor(0.5 * e8), n6 = l5 + e8), h5 > e8 && (o3 = o3 + Math.floor(0.5 * h5) - Math.floor(0.5 * e8), a6 = o3 + e8);
    }
    for (let m7 = l5; m7 < n6; m7++) for (let e8 = o3; e8 < a6; e8++) s10.push([0, m7, e8]);
    return null != t4 && (t4[0] = this.origin[0] + o3 * r6[0], t4[1] = this.origin[1] - n6 * r6[1], t4[2] = this.origin[0] + a6 * r6[0], t4[3] = this.origin[1] - l5 * r6[1]), s10;
  }
  ...
  static fromExtent(e7, t4) {
    const i4 = e7[2] - e7[0], s10 = e7[3] - e7[1], l5 = Z(t4), o3 = 1.2 * Math.max(i4, s10), a6 = 256, h5 = o3 / a6, c6 = h5 * l5 * (96 / 0.0254), m7 = new _f(new j2({ size: [a6, a6], origin: new _({ x: e7[0] - 0.5 * (o3 - i4), y: e7[3] + 0.5 * (o3 - s10) }), lods: [new l2({ level: 0, resolution: h5, scale: c6 })], spatialReference: t4 }));
...
var l4 = S(q / 10);
var m5 = u();
f3.WebMercatorAuxiliarySphere.getExtent(0, 0, 0, m5);
var c3 = u([-180, -90, 180, 90]);

// node_modules/@arcgis/core/chunks/terrainUtilsPlanar.js
function s8(t4, o3, l5, s10) {
  if (null == t4) return x();
  const a6 = t4.spatialReference;
  if (a6.isGeographic && !i3(a6)) return new s("tilingscheme:local-unsupported-spatial-reference", "The tiling scheme spatial reference is not supported in local scenes");
  ...
  const p3 = c4(t4, l5);
  return p3 || (null == o3 || a6.equals(o3) || o3.isWGS84 && a6.isWebMercator ? null : new s("tilingscheme:spatial-reference-mismatch", "The tiling scheme does not match the spatial reference of the local scene"));
}
function c4(r6, n6) {
  const s10 = r6.lods, c6 = s10[0].resolution * 2 ** s10[0].level, a6 = [c6 * r6.size[0], c6 * r6.size[1]], u4 = [r6.origin.x, r6.origin.y], p3 = o(n6), f7 = u();
  f3.computeRowColExtent(p3, a6, u4, f7);
  const m7 = (f7[2] - f7[0]) * (f7[3] - f7[1]);
  if (m7 > s7) {
    const t4 = s10[0].scale * 2 ** s10[0].level;
    let o3 = Math.max((p3[3] - p3[1]) / r6.size[1], (p3[2] - p3[0]) / r6.size[0]) * t4 / c6;
...
function n4(n6, r6, c6, o3) {
  if (null == n6) return x();
  const s10 = n6?.lods.length - 1, a6 = n6.spatialReference;
  if (a6.isWebMercator) {
    ...
    if (!o2(a6)) return new s("tilingscheme:global-unsupported-spatial-reference", "The tiling scheme spatial reference is not supported in global scenes");
...
async function h3(e7, a6, i4) {
  ...
  if (b3(r6)) {
    try {
      ...
      const e8 = "basemap-compatibility:unknown-error", a7 = "Unknown basemap compatibility error", { name: i5 = e8, message: r7 = a7, details: n6 } = s10;
...
function g2(e7, a6) {
  const i4 = a6.state.viewingMode;
  if (!i4) return;
  let r6, n6;
  if ("wmts" === e7?.type) {
    const s10 = k2(e7, a6.spatialReference, i4);
  ...
  if (i4 === l3.Global) {
    let a7 = M(r6, n6, null, i4);
    if (a7 && "vector-tile" === e7?.type && null != n6 && f7 && !M(f7, n6, null, i4) && (a7 = null), a7) {
      const e8 = r6.spatialReference.isWebMercator ? "web-mercator" : "wgs84";
      throw new s(`basemapgalleryitem:tiling-scheme-unsupported-${e8}-global`, "Basemap tiling scheme is unsupported in global mode", { error: a7 });
    }
  ...
  const u4 = a6.basemapTerrain?.tilingScheme;
...
var a5 = V.ofType(O3);
var c5 = class extends b2 {
  constructor(o3) {
    super(o3), this.basemaps = new a5();
  }
  destroy() {
...
r([m({ type: a5 })], c5.prototype, "basemaps", void 0), r([m({ readOnly: true })], c5.prototype, "state", null), c5 = r([a("esri.widgets.BasemapGallery.support.LocalBasemapsSource")], c5);
var m6 = c5;

// node_modules/@arcgis/core/widgets/BasemapGallery/support/PortalBasemapsSource.js
var d3 = V.ofType(O3);
var y3 = class extends p.LoadableMixin(m3.EsriPromiseMixin(m6)) {
  constructor(t4) {
  ...
  async refresh() {
    ...
    const t4 = this.portal, a6 = new AbortController();
    this._lastPortalBasemapFetchController = a6, this.notifyChange("state");
    try {
      const s10 = await t4.fetchBasemaps(this._toQueryString(this.query), { signal: a6.signal, include3d: "3d" === this.viewType || void 0 });
  ...
  _toQueryString(t4) {
    return t4 && "string" != typeof t4 ? Object.keys(t4).map((a6) => `${a6}:${t4[a6]}`).join(" AND ") : t4;
  }
  async _updateBasemaps(t4 = []) {
    let a6 = await this._filterBasemaps(t4);
    a6 = this.updateBasemapsCallback ? await this.updateBasemapsCallback(a6) : a6, this.basemaps.removeAll(), this.basemaps.addMany(a6);
  }
  async _filterBasemaps(t4) {
    if (!this.filterFunction) return t4;
    const a6 = t4.map(this.filterFunction), s10 = await Promise.all(a6);
    return t4.filter((t5, a7) => s10[a7]);
  }
};
...
var E = class extends p {
  ...
  castSource(e7) {
    return Array.isArray(e7) || V.isCollection(e7) ? new m6({ basemaps: Array.isArray(e7) ? new V(e7) : e7 }) : _2(e7) ? new f5({ portal: e7 }) : I(e7) ? new f5(e7) : R(e7) ? e7 : null;
  }
  get state() {
  ...
  _recreateItems() {
    const e7 = this.source?.basemaps ?? [], { view: t4, compatibilityFunction: s10 } = this, i4 = new Map(this.items.map((e8) => [e8.basemap, e8])), a6 = e7.map((e8) => {
      const a7 = i4.get(e8);
      return a7 ? (i4.delete(e8), a7) : new h4({ basemap: e8, compatibilityFunction: s10, view: t4 });
    });
    this.items.removeAll(), this.items.addMany(a6), i4.forEach((e8) => e8.destroy());
  }
  _updateItems() {
...
var w3 = class extends O {
  ...
  loadDependencies() {
    return c({ scrim: () => import("./calcite-scrim-K6XWXI4L.js"), chip: () => import("./calcite-chip-WULQ3PZX.js") });
  }
  get activeBasemap() {
  ...
  _handleKeyDown(e7) {
    ...
    e7.preventDefault();
    const { items: t4, activeBasemapIndex: a6 } = this.viewModel, i4 = "ArrowUp" === s10 || "ArrowLeft" === s10 ? this._getRoundRobinIndex(Math.max(a6 - 1, -1), t4.length) : this._getRoundRobinIndex(a6 + 1, t4.length), r6 = t4.at(i4);
  ...
  _renderBasemapGalleryItem(e7, t4) {
    const a6 = j3(e7.basemap) || n2("esri/themes/base/images/basemap-toggle-64.svg"), i4 = e7.basemap.title, r6 = e7.basemap.portalItem?.snippet, o3 = e7.error?.message || r6 || i4, { viewModel: { state: d4, activeBasemapIndex: p3 } } = this, m7 = this.disabled || "disabled" === d4, h5 = p3 === t4, g4 = h5 || -1 === p3 && 0 === t4 ? 0 : -1, v2 = "loading" === d4, b5 = { [t3.selectedItem]: h5, [t3.itemError]: "error" === e7.state }, w4 = `basemapgallery-item-${e7.uid}`;
    return n("li", { afterUpdate: this._focusBasemapItem, "aria-checked": h5.toString(), "aria-disabled": m7.toString(), "aria-labelledby": w4, bind: this, class: this.classes(t3.item, b5), "data-item": e7, key: e7.uid, onclick: this._handleClick, onkeydown: this._handleClick, role: "radio", tabIndex: g4, title: o3 }, n("img", { alt: "", class: t3.itemThumbnail, src: a6 }), n("div", { class: t3.itemContent, key: "content" }, n("div", { class: t3.itemTitle, key: "title" }, n("span", { id: w4 }, i4)), u2(e7.basemap) ? this._renderTags(e7.basemap) : null), "loading" === e7.state || h5 && v2 ? n("calcite-scrim", null, n("span", { "aria-hidden": "true", class: e2.loaderAnimation, key: "loader", role: "presentation" })) : null);
  }
  _renderTags(e7) {
