...
var S2 = class extends e4 {
  ...
  setTransform(t2) {
    ...
    M(e5, e5, r2(i2, a3)), M(e5, e5, r2(d / 2, c / 2)), h(e5, e5, -_2), M(e5, e5, r2(-d / 2, -c / 2)), b(e5, e5, r2(d, c)), i(this.transforms.displayViewScreenMat3, t2.displayViewMat3, e5);
  }
  ...
  _createTexture(t2) {
    ...
    return i2.internalFormat = e5 ? B.RGBA8 : M2.RGBA, i2.wrapMode = P.CLAMP_TO_EDGE, i2.isImmutable = e5, i2.width = this._sourceWidth, i2.height = this._sourceHeight, new S(t2, i2);
  }
};

// node_modules/@arcgis/core/views/2d/engine/webgl/shaderGraph/techniques/shaders/bitmapUtils.js
function n(t2) {
  const u = new _(1 / 6), d = t2.multiply(t2), i2 = d.multiply(t2), n2 = u.multiply(i2.multiply(-1).add(new _(3).multiply(d)).subtract(new _(3).multiply(t2)).add(1)), p3 = u.multiply(i2.multiply(3).subtract(d.multiply(6)).add(4)), m4 = u.multiply(i2.multiply(-3).add(d.multiply(3)).add(t2.multiply(3)).add(1)), a3 = u.multiply(i2);
  return new Y(n2, p3, m4, a3);
}
function p(t2) {
  const u = n(t2), d = u.x.add(u.y), i2 = u.z.add(u.w), p3 = new _(1).subtract(u.y.divide(d)).add(t2), m4 = new _(1).add(u.w.divide(i2)).subtract(t2);
  return new Y(p3, m4, d, i2);
}
function m2(y, n2, m4) {
  const a3 = new C2(new _(1).divide(m4.x), 0), e5 = new C2(0, new _(1).divide(m4.y)), r3 = n2.multiply(m4).subtract(0.5), c = p(Te(r3).x).xyz, s = p(Te(r3).y).xyz;
  let w5 = n2.add(c.x.multiply(a3)), o2 = n2.subtract(c.y.multiply(a3));
  const x = w5.add(s.x.multiply(e5)), b3 = o2.add(s.x.multiply(e5));
  w5 = w5.subtract(s.y.multiply(e5)), o2 = o2.subtract(s.y.multiply(e5));
  let z = yn(y, o2), f4 = yn(y, w5);
  const v3 = yn(y, b3), g3 = yn(y, x);
  return z = Ye(z, v3, s.z), f4 = Ye(f4, g3, s.z), z = Ye(z, f4, c.z), z;
}
...
var m3 = (e5, i2, t2) => {
  if ("dynamic" === t2.samplingMode) {
    const { state: t3 } = e5, s = i2.resolution / i2.pixelRatio / t3.resolution, o2 = Math.round(e5.pixelRatio) !== e5.pixelRatio, n2 = s > 1.05 || s < 0.95;
    return t3.rotation || n2 || o2 || i2.isSourceScaled || i2.rotation ? a2.bilinear : a2.nearest;
  }
...
var p2 = class extends t {
  ...
  render(i2, t2) {
    const { context: s, renderingOptions: r3, painter: p3 } = i2;
    for (const c of t2.bitmaps) {
      const t3 = c.texture;
      if (!c.source || !c.isReady || null == t3) continue;
      const l2 = m3(i2, c, r3);
      i2.timeline.begin(this.name), p3.setPipelineState({ depth: false, stencil: { test: { mask: 255, ref: c.stencilRef, compare: C.EQUAL, op: { fail: D.KEEP, zFail: D.KEEP, zPass: D.KEEP } }, write: false }, color: { write: [true, true, true, true], blendMode: "additive" === c.blendFunction ? "additive" : "composite" } }), c.setSamplingProfile(l2);
      ...
      p3.submitDrawMesh(s, { shader: this.shaders.bitmap, uniforms: { config: h2 }, defines: { bicubic: l2 === a2.bicubic }, optionalAttributes: null, useComputeBuffer: false }, p3.quadMesh), i2.timeline.end(this.name);
    }
  }
};
...
//# sourceMappingURL=chunk-IEBPGCZ4.js.map
