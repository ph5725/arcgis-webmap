...
interface SimpleOptions {
  // Có thể thêm các thuộc tính tùy chọn nếu cần
}
...
export const PrivatePortalMap: React.FC<PanelProps<SimpleOptions>> = ({ width, height }) => {
  ...
  return (
    <div style={{ height: height ?? '100vh', width: width ?? '100%' }}>
      ...
      {mapLoaded && (
        ...
        >
          {showInterface ? (
            <span style={{ fontSize: '10px' }}><LuMonitor size={20} color="#333" /></span>
          ) : (
            <span style={{ fontSize: '10px' }}><LuMinimize size={20} color="#333" /></span>
          )}
        </button>
      )}
